export const timeCounter = (poczatkowyCzas: string, koncowyCzas: string) => {
    const [poczatkoweGodziny, poczatkoweMinuty] = poczatkowyCzas.split(':').map(Number);
    const [koncoweGodziny, koncoweMinuty] = koncowyCzas.split(':').map(Number);
  
    const roznicaMinut = (koncoweGodziny * 60 + koncoweMinuty) - (poczatkoweGodziny * 60 + poczatkoweMinuty);
